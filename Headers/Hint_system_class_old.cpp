													...
													std::vector<Pose_estimation_results> &pose_results )
{
	...
	for (int i = 0; i < object_names.size(); i++)
	{
		...
		for (int j = 0; j < view_indices[i].size(); j++)
		{
			per.identified_views.push_back (view_indices[i][j]);
		}
		
		per.initial_view = view_indices[i][0];
...
/**
  Checks if the z-axis of the initial view is aligned with the z-axis of the camera
  @param per The pose estimation result
  @return True if there is a misalignment and false otherwise
*/
bool
Hint_system_class::z_axis_misalignment (Pose_estimation_results per)
{
	// Extraxt and normalize z-axis from intial view
	Eigen::Vector3d z_axis_initial_view = per.graph.get_rotation (per.initial_view).block<3,1> (0,2);
	z_axis_initial_view.normalize ();
	
	// Check angle between z-axis of initial view and z-axis of camera
	if (z_axis_initial_view (2) < Z_AXIS_MISALIGNMENT_THRESHOLD)
	{
		// z-axis of initial view and z-axis of camera are not aligned!
		return true;
	}
	
	// No misalignment between the z-axis of initial view and z-axis of camera 
	return false;
}

/**
  Marks neighboring nodes to the current camera position as invalid. Ideally we should not visit positions that have already been visited.
*/
void
Hint_system_class::disable_neighboring_nodes (Pose_estimation_results &per)
{
	for (int i = 0; i < per.graph.get_size(); i++)
	{		
		if (i == per.initial_view)
		{
			continue;
		}
		
		Eigen::Vector3d z_axis = per.graph.get_rotation (i).block <3,1> (0,2);
		z_axis.normalize ();
		double angle = acos (z_axis (2)) * (180 / PI_CONST);
		if (std::abs (angle) < VALID_AXIS_THR)
		{
			per.graph.set_valid_node (i, false);
		}
	}
}
											...
											std::vector<Pose_estimation_results> &invalid_pose_results )
{
	...
	for (int i = 0; i < pose_results.size(); i++)
	{
		...
		pose_results[i].graph.find_valid_nodes (plane, previous_positions, VALID_AXIS_THR);
		...
		// Check if the initial (current) view is valid
		std::vector<bool> valid_nodes = pose_results[i].graph.get_valid_nodes ();
		if (!valid_nodes[pose_results[i].initial_view])
		{
			if (print_info_)
			{
				std::cout << "The initial view is below the plane!\n" << std::endl; 
			}
			
			pose_results[i].invalid_viewpoint_position = true;
		...
		// Check if the z-axis of the initial view is aligned with the z-axis of the camera
		if (z_axis_misalignment (pose_results[i]))
		{
			if (print_info_)
			{
				std::cout << "The z-axis of the initial view is not aligned with the z-axis of the camera!\n" << std::endl; 
			}
			
			pose_results[i].invalid_z_axis = true;
			invalid_pose_results.push_back (pose_results[i]);
			continue;
		}
		
		// The pose is valid! Mark neighboring nodes to the current camera as invalid and add pose to valid pose results.
		disable_neighboring_nodes (pose_results[i]);
...
Hint_system_class::sort_valid_pose_results (std::vector<Pose_estimation_results> &valid_pose_results)
{
...
Hint_system_class::view_search_results (std::string scene_name, Pose_estimation_results &per)
{
	...
	r_vec_search[per.initial_view] = 1.0;
	g_vec_search[per.initial_view] = 1.0;
	b_vec_search[per.initial_view] = 1.0;
	r_vec_search[better_view_] = 0.0;
	...
	for (int i = 0; i < valid_nodes.size(); i++)
	{
		if (valid_nodes[i] && i != per.initial_view && i != better_view_)
		{
	...
	for (int i = 0; i < previous_positions.size(); i++)
	{
		...
		visu_ptr->addCube (-0.005, 0.005, -0.005, 0.005, -0.005, 0.005, 1.0, 1.0, 1.0, "cube", 0);
	}
									...
									int flag )
{
	...
	for (int i = 0; i < invalid_pose_results.size(); i++)
	{
		...
		if (invalid_per.invalid_viewpoint_position)
		{
			ss << " Invalid viewpoint!";
		}
		else if (invalid_per.invalid_pose_position)
		{
			ss << " Invalid pose position!";
		}
		else if (invalid_per.invalid_z_axis)
		{
			ss << " Invalid z-axis of identified view!";
		}
...
Hint_system_class::find_new_view (std::string scene_name, int cluster_index)
{
	...
	std::vector<Pose_estimation_results> valid_pose_results;
	std::vector<Pose_estimation_results> invalid_pose_results;
	if (camera_validation_test (scene_name, pose_results, valid_pose_results, invalid_pose_results) > 0)
	{
	...
	std::vector<std::string> similar_model_names = sort_valid_pose_results (valid_pose_results);
